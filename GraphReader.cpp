...
Graph* GraphReader::readGraphFile(const std::string& filePath) {
    ...
    while (std::getline(file, line)) {
        ...
        if (line.find("tipo:") != std::string::npos) {
            if (line.find("lista") != std::string::npos) rep = Representation::LIST;
            if (line.find("matriz") != std::string::npos) rep = Representation::MATRIX;
        }
    ...
    for (const std::string& nodeName : uniqueNodes) {
        dynamicGraph->appendNode(nodeName);
    }

    for (const auto& edge : temporaryEdges) {
        dynamicGraph->appendEdge(edge.origin, edge.target, edge.weight);
    }
